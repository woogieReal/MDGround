 ...
 * 게시물 등록 시 업로드한 이미지들의 이름을 보고 썸네일을 선택하는 것이 기존의 방식으로, 이는 이미지 이름을 일일히 확인 하여야 해서 불편하다는 동료 개발자 의견 **(2021-05-19)**
 ...
 * 게시물 등록 화면에서 썸네일 이미지를 선택할 때 이미지의 크기가 작아 선택이 어려움 **(2021-05-30)**
   + 이미지를 클릭하면 이미지 크기가 확대되고, 다시 한 번 클릭하면 원래의 작은 크기로 돌아가도록 함 **(2021-05-31)**
 * 화면 크기 조정 **(2021-05-31)**
   + 게시물 조회 화면 크기 최대 900px로 확대
   + 마이 페이지 화면 크기 최대 800px로 확대
 * 카테고리 추가(typescript, react, next) **(2021-05-31)**
 * 카테고리 추가(programming) **(2021-06-25)**
 * ###(h3), ####(h4), #####(h5)으로 작성한 것들이 각각 명확히 구분되지 않음 **(2021-07-02)**
   + 아래와 같이 각각 색을 추가하였음 **(2021-07-18)**
     + ###(h3) : 짙은 빨강 (color: #c62828;)
     + ####(h4) : 짙은 파랑 (color: #0d47a1;)
     + #####(h5) : 녹색 (color: green;)
 * 리스트를 작성할 경우 리스트 간 거리가 가까워 시인성과 가독성이 떨어짐 **(2021-07-05)**
   + li 에 `margin-bottom: 5px;` 을 기본적으로 적용하여 리스트 간 높이를 조정 **(2021-07-18)**
   + 리스트 아래에 리스트를 추가적으로 작성하는 경우 부모리스트 간의 구별을 쉽게 하도록 하기 위해 `li > ul, ol { margin-bottom: 20px; }` 속성 추가 **(2021-07-18)**
